...
export default function ReactButton({ post }: PostProps) {
  ...
  const handleToggleReact = (content: AddReactionInput['emoji']) => {
    ...
    return <Popover className="relative">
        <Popover.Button>
            <Button  variant={'outline-ghost'} centerItems shape={'circle'} iconLeft={<PiChatCircleBold />} className="text-white border-white/50 sm:border-stroke sm:text-black bg-black/50 sm:bg-transparent focus:outline-none"/>
        </Popover.Button>
        <Transition
            as={Fragment}
