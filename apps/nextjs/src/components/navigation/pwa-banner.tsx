    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (bannerHidden !== 'true') {
                if (!lastShownTime || Date.now() - parseInt(lastShownTime) > 72 * 60 * 60 * 1000) {
                    setShowBanner(true);
        ...
        return (
            <div className="z-50 justify-between flex-col h-screen w-screen bg-white dark:bg-black text-black dark:text-white flex py-10 items-center px-6 text-center overflow-y-auto">
                ...
                <div className="flex flex-col gap-1">
                    <p className="flex gap-1 items-center">1. Tap the <span><Share className=" text-[#007AFF] w-4 h-4" /></span> Share button below</p>
                    <p>2. Tap "Add to Home Screen"</p>
                    <p>3. Tap "Add"</p>
                </div>

                <div className="flex flex-col gap-3 items-center">
                    <p className="text-muted-foreground text-sm text-underline cursor-pointer" onClick={handleRemindLater}>Remind me later</p>
                    <p className="text-muted-foreground/75 text-sm cursor-pointer" onClick={handleDontShowAgain}>Don't show this again</p>
