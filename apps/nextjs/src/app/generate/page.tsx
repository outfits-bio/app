...
import { PiArrowsClockwiseBold, PiXBold, PiFloppyDiskBold, PiListBold } from "react-icons/pi";
import { motion, AnimatePresence } from "framer-motion";
import { BaseModal, BaseModalContent, BaseModalHeader, BaseModalTitle, BaseModalFooter, BaseModalTrigger, BaseModalDescription } from "~/components/modals/base-modal";
...
const pieceOrder = ["HEADWEAR", "GLASSES", "JEWELRY", "HOODIE", "SHIRT", "WATCH", "PANTS", "SHOES"];
...
export default function GenerateOutfitPage() {
    ...
    return (
        <div className="container mx-auto p-4">
            <h1 className="text-2xl font-bold mb-1 font-clash">Generate Your Outfit [BETA]</h1>
            <p className="text-sm text-gray-500  mb-4">This feature is in testing, outfits are stored on your device. They are not shared across devices and might lose them in the future.</p>
            <div className="flex mb-4 flex-wrap text-nowrap gap-3">
                ...
                <BaseModal>
                    ...
                    <BaseModalContent>
                        ...
                        <p className="text-sm text-gray-500">This feature is in testing, outfits are stored on your device. They are not shared across devices and might lose them in the future.</p>
                    </BaseModalContent>
                </BaseModal>
                <BaseModal>
                    <BaseModalTrigger>
                    ...
                    <BaseModalContent>
                        ...
                        <BaseModalDescription>
                            This feature is in testing, outfits are stored on your device. They are not shared across devices and might lose them in the future.
                        </BaseModalDescription>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <BaseModalFooter>
                                {savedOutfits.length === 0 && (
                                    <p className="text-sm w-full text-gray-500">No saved outfits</p>
                                )}
                            </BaseModalFooter>
                        </div>
                    </BaseModalContent>
            ...
            {hasNoPieces && (
                <p className="mt-4 text-red-500">You need to post some pieces before you can generate an outfit.</p>
            )}
            <AnimatePresence>
                {outfit.length > 0 && (
                    <motion.div
                                    ...
                                    >
                                        <div className="flex flex-col items-center space-y-2">
                                            <Button
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    regeneratePieceMutation.mutate({ type: piece.type as "HOODIE" | "SHIRT" | "PANTS" | "SHOES" | "WATCH" | "GLASSES" | "HEADWEAR" | "JEWELRY" });
                                                }}
                                                iconLeft={<PiArrowsClockwiseBold className="w-4 h-4 mr-2" />}
                                                shape="square"
                                            />
                                            <Button
                                                variant="ghost"
