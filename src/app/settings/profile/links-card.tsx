...
export function LinksCard() {
    ...
    const { mutate: addLink, isPending: linkPending } = api.user.addLink.useMutation({
        onSuccess: async () => {
            resetField("url");
    ...
    const { mutate: removeLink, isPending: removeLinkPending, variables: removeLinkVariables } = api.user.removeLink.useMutation({
        onSuccess: async () => {
            await ctx.user.getMe.refetch();
                        ...
                        <div className='flex items-center gap-2 w-full' key={link.id}>
                            ...
                            <div>
                                <Button variant='outline' iconLeft={<PiTrash />}
                                    centerItems
                                    isLoading={removeLinkPending && removeLinkVariables?.id === link.id}
