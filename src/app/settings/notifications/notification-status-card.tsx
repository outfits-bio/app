...
export function NotificationStatusCard() {
  ...
  const subscribeToPushNotifications =
    api.notifications.subscribeToPushNotifications.useMutation();
  ...
  const subscribeUser = async () => {
    try {
      ...
      const result = await subscribeToPushNotifications.mutateAsync({
    ...
    <div className="md:hidden flex flex-col items-start rounded-lg border border-stroke bg-white dark:bg-black">
      <div className="flex-wrap items-start flex gap-24 p-10 self-stretch">
        <div className="flex flex-col items-start gap-3 flex-1">
          ...
          <p>
            Turn on notifications to receive notifications on your mobile
            device.
          </p>
        </div>
        <div
          className={`flex w-[96px] h-[48px] p-2 items-center gap-3 rounded-full border dark:border-stroke ${notificationsEnabled && "bg-orange-accent justify-end"}`}
          onClick={handleNotificationStatus}
        >
          <div
            className={`w-[32px] h-[32px] flex-shrink-0 bg-black rounded-full ${notificationsEnabled && "bg-white"} dark:bg-white`}
          />
        </div>
      </div>
