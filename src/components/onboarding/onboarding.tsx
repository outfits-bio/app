...
export const Onboarding = ({
  ...
  const { data, error } = api.user.getMe.useQuery(undefined, {
  ...
  useEffect(() => {
    if (error) {
      handleErrors({ e: error, message: "Failed to fetch you!" });
    }
    if (data) {
  ...
  }, [data, error, setValue, setFileUrl]);
  ...
  const { mutate: setImage } = api.user.setImage.useMutation({
    onSuccess: async (result) => {
      ...
      if (
        getValues("tagline") === data?.tagline &&
        getValues("username") === data?.username
      ) {
        await update();
        setOnboardingStarted(2);
      }
    ...
    <>
      ...
      <div className="w-full h-full flex lg:px-56 lg:w-auto flex-col py-4 sm:justify-center items-center gap-4">
        {onboardingStarted === 1 ? (
          ...
          >
            <h1 className="hidden sm:block sm:text-5xl font-black font-clash">
              Show the world who you really are.
            </h1>

            <div className="flex gap-2 w-full flex-col sm:flex-row items-center">
              <div
                className="rounded-full h-64 w-64 sm:h-44 sm:w-44 flex items-center justify-center border"
              ...
              <div className="flex grow h-full flex-col gap-2 w-full sm:w-auto mt-11 sm:mt-auto">
                {errors.tagline && errors.username && (
            ...
            <div className="w-full flex gap-2 mt-4">
              <Button
                ...
                onClick={() => setOnboardingStarted(0)}
                centerItems
              >
                Back
              </Button>
          ...
          <OnboardingAppearance
            ...
            setOnboardingStarted={setOnboardingStarted}
            onboardingStarted={onboardingStarted}
          />
        ) : (
          <OnboardingStartSection
            username={username}
            setOnboardingStarted={setOnboardingStarted}
          />
        )}
