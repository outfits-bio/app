...
export function UserTagInput({
  ...
  const [suggestions, setSuggestions] = useState<
    Array<{ username: string; image: string | null; id: string }>
  >([]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && input.trim()) {
      ...
      if (
        ...
        suggestions.some((s) => s.username === newTag)
      ) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {value.map((tag) => (
        <span
          key={tag}
          className="bg-gray-200 px-2 py-1 rounded-full text-sm items-center flex"
        >
          {tag}
