...
export const AdminEditUserModal = (props: AdminEditUserModalProps) => {
  ...
  const { mutate, isPending } = api.admin.editUser.useMutation({
    onSuccess: async () => {
      await ctx.user.getProfile.refetch({
  ...
  const { mutate: removeImage, isPending: removeImagePending } =
    api.admin.removeUserAvatar.useMutation({
      onSuccess: async () => {
        await ctx.user.getProfile.refetch({
          username: props.targetUser.username ?? "",
        });
  ...
  const {
    ...
    variables,
  } = api.admin.removeUserLink.useMutation({
    onSuccess: async () => {
      await ctx.user.getProfile.refetch({
        username: props.targetUser.username ?? "",
      });
  ...
  const { mutate: giveVerified, isPending: giveVerifiedPending } =
    api.admin.toggleUserVerified.useMutation({
      onSuccess: async () => {
        await ctx.user.getProfile.refetch({
          username: props.targetUser.username ?? "",
        });
          ...
          >
            <div className="flex flex-col gap-2">
              ...
              <Button
                ...
                type="button"
                centerItems
                isLoading={removeImagePending}
                onClick={() => removeImage({ id: props.targetUser.id })}
              >
                Remove Avatar
              </Button>
              {
                <Button
                  variant={"outline-ghost"}
                  type="button"
                  isLoading={giveVerifiedPending}
                  centerItems
                  onClick={() => giveVerified({ id: props.targetUser.id })}
                >
                  {props.targetUser.verified
                    ? "Revoke Verified"
                    : "Give Verified"}
                </Button>
              }
              ...
              <ul className="text-left w-full flex flex-col items-start gap-2">
                {props.targetUser.links?.map((link) => (
                  <button
                    type="button"
                    key={link.id}
                    className="text-sm flex items-center font-semibold py-2 border-stroke rounded-xl border w-full text-left px-4 hover:bg-hover"
                    onClick={() =>
                      removeLink({ id: link.userId, linkId: link.id })
                    }
                  >
                    {link.url}
                    {removeLinkPending && variables?.linkId === link.id ? (
                      <PiSpinnerGap className="animate-spin ml-auto" />
                    ) : (
                      <PiTrashSimple className="ml-auto" />
                    )}
                  </button>
                ))}
              </ul>
