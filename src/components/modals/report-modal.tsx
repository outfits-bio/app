...
export const ReportModal = ({ type, id }: ReportModalProps) => {
  ...
  const { mutate } = api.report.report.useMutation({
    onError: (e) =>
      handleErrors({ e, message: "An error occurred while reporting this." }),
  ...
  return (
    <BaseModal>
      ...
      <BaseModalContent>
        ...
        <BaseModalDescription>
          Report {type === "USER" ? "this user" : "this post"} for potentially
        ...
        <form onSubmit={handleSubmit(handleReportSubmit)}>
          ...
          <div className="flex w-full gap-2">
            <BaseModalClose>
              <Button variant="outline" centerItems type="button">
                Cancel
              </Button>
            </BaseModalClose>
            <Button centerItems type="submit">
              Report
            </Button>
          </div>
        </form>
