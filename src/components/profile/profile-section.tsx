...
export const ProfileCard = ({ profileData, username }: Props) => {
  ...
  const { data: lanyardData } = api.user.getLanyardStatus.useQuery({
    username,
  });
  ...
  const { mutate, isPending } = api.user.likeProfile.useMutation({
    onMutate: async () => {
      ...
      ctx.user.getProfile.setData(
        { username },
        {
          ...previousProfileData,
          authUserHasLiked: !previousProfileData.authUserHasLiked,
          likeCount:
            previousProfileData.likeCount +
            (previousProfileData.authUserHasLiked ? -1 : 1),
        },
      );
    ...
    onError: (e) =>
      handleErrors({
        ...
        fn: () => {
          ...
          ctx.user.getProfile.setData(
            { username },
            { ...profileData, authUserHasLiked: !profileData.authUserHasLiked },
          );
        },
      }),
  });
  ...
  const userUrl =
    typeof window !== "undefined" ? `${origin}${window.location.pathname}` : "";
  ...
  return (
    <div className="md:h-full flex flex-col font-satoshi md:bg-white md:dark:bg-black md:border-r border-stroke pl-4 py-4 md:px-12">
      ...
      <div className="md:w-96 w-full flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1 md:gap-4">
            <h1 className="font-black text-2xl md:text-4xl font-clash gap-2 md:gap-3 flex items-center">
              ...
              <div className="group relative w-max">
                {profileData?.admin ? (
                  <PiHammerBold className="w-6 h-6 md:w-8 md:h-8" />
                ) : (
                  profileData?.verified && (
                    <PiSealCheckBold className="w-6 h-6 md:w-8 md:h-8" />
                  )
                )}
                ...
                <span className="shadow-lg pointer-events-none absolute bg-black dark:bg-white -top-[42px] -left-full w-max rounded-xl p-2 opacity-0 transition-opacity group-hover:opacity-100">
                  <p className="text-sm font-medium text-white dark:text-black">
                    {profileData?.admin
                      ? "Administrator"
                      : profileData?.verified && "Verified"}
                  </p>
                </span>
              </div>
            </h1>
            ...
            <div className="flex gap-4 text-sm md:text-base">
              <p className={`flex items-center gap-1`}>
                ...
                <span>
                  <span className="font-bold">{profileData?.imageCount}</span>{" "}
                  Post{profileData?.imageCount !== 1 ? "s" : ""}
                </span>
              </p>
                ...
                <span className="cursor-pointer">
                  <p className="flex items-center gap-1">
                    ...
                    <span>
                      ...
                      Follower{profileData?.likeCount !== 1 ? "s" : ""}
                    </span>
                  </p>
                </span>
              </FollowersModal>
            ...
            <Link href={`${link.url}`} key={link.id}>
              <p className="flex items-center gap-1">
                {link.type === LinkType.TWITTER && (
                  <PiTwitterLogoBold className="w-5 h-5" />
                )}
                {link.type === LinkType.YOUTUBE && (
                  <PiYoutubeLogoBold className="w-5 h-5" />
                )}
                {link.type === LinkType.TIKTOK && (
                  <PiTiktokLogoBold className="w-5 h-5" />
                )}
                {link.type === LinkType.DISCORD && (
                  <PiDiscordLogoBold className="w-5 h-5" />
                )}
                {link.type === LinkType.INSTAGRAM && (
                  <PiInstagramLogoBold className="w-5 h-5" />
                )}
        ...
        {profileData?.lanyardEnabled &&
          !data?.user.hideLanyard &&
              ...
              {lanyardData?.albumArt ? (
                <>
                  <div className="relative w-6 h-6">
                    <Image
                      src={lanyardData.albumArt}
                      alt={lanyardData.title}
                      className="rounded-full animate-spin-cd border"
                      width={40}
                      height={40}
                    />
                  </div>
                  ...
                  >
                    <p className="text-sm mx-4">
                      Listening to{" "}
                      <span className="font-bold">{lanyardData?.title}</span> by{" "}
                      <span className="font-bold">{lanyardData?.artist}</span>
                    </p>
                  </Marquee>
                </>
              ) : null}
              {lanyardData === null && isCurrentUser && (
                <SpotifySetupModal>
                  <p className="text-sm text-error flex gap-2 items-center cursor-pointer hover:underline">
                    More setup required to display Spotify.{" "}
                    <PiQuestion className="w-4 h-4" />
                  </p>
                </SpotifySetupModal>
              )}
            </div>
