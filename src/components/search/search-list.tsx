...
export function SearchList() {
  ...
  const {
    ...
    isFetchingNextPage,
    hasNextPage,
  } = api.user.searchProfiles.useInfiniteQuery(
    { username: input },
    {
      enabled: !!username,
    ...
    <div className="w-full md:w-5/6 lg:w-3/4 xl:w-1/2">
      ...
      {input.length > 0 && (
        <div className="flex flex-col gap-2">
          {(users?.length ?? 0) > 0 ? (
                  ...
                  <div className="flex flex-col gap-1">
                    <h1 className="font-black flex gap-1 items-center">
                      ...
                      {user?.admin ? (
                        ...
                        user?.verified && <PiSealCheck className="w-4 h-4" />
                      )}
                    </h1>
            ...
            <div className="bg-white dark:bg-black border border-stroke p-4 rounded-xl">
              No results
            </div>
          )}
        </div>
      ...
      {hasNextPage && (
        <Button
          ...
          className="mt-2 border border-stroke"
        >
          Load More
        </Button>
      )}
