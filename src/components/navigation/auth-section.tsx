...
const NotificationsMenu = dynamic(
  () =>
    import("../menus/notifications-menu").then((mod) => mod.NotificationsMenu),
  {
    loading: () => (
...
export function AuthSection() {
  ...
  const iconComponent =
    pathname === "/notifications" ? (
      <PiBellSimpleFill className="h-5 w-5" />
    ) : (
  ...
  if (session && session.user) {
    const { data: notificationsCount } =
      api.notifications.getUnreadNotificationsCount.useQuery(undefined, {
        enabled: !!session,
      });
