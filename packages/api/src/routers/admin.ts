import {
  ...
  deleteUserSchema,
  editUserSchema,
} from "@acme/validators/admin.schema";
import { TRPCError } from "@trpc/server";
import { NotificationType } from "@acme/db";
import { createTRPCRouter, protectedProcedure } from "../trpc";

import { deleteImage } from "@acme/utils/image.util";
...
export const adminRouter = createTRPCRouter({
  ...
  removeUserAvatar: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const oldImage = await ctx.db.user.findUnique({
        where: { id },
        select: { image: true },
      });
  ...
  toggleUserVerified: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        await ctx.db.$transaction([user, notification]);
      } catch (error) {
        await ctx.db.user.update({
          where: {
            id,
