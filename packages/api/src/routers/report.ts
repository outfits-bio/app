...
export const reportRouter = createTRPCRouter({
  report: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (type === "USER") {
        const report = await ctx.db.user.update({
        ...
        await axios.post(process.env.DISCORD_MOD_REPORT_WEBHOOK_URL ?? "", {
          username: "Reports Bot",
          avatar_url: "",
          content: `New report from [${ctx.session.user.username}](https://outfits.bio/${encodeURI(ctx.session.user.username)})`,
          embeds: [
            {
              title: "Report",
        ...
        const report = await ctx.db.post.update({
          where: { id: id as string },
          data: {
            reports: {
              create: {
